...
public class UpdateUserController {
    ...
    public void setUser(Utilisateur user) {
        ...
        securityQuestionComboBox.setValue(user.getQuestionSecurite());
        securityAnswerField.setText(user.getReponseSecurite());
    }
    ...
    private void handleUpdate() {
        ...
        // Update the user object with all fields
        ...
        user.setMotDePasseUtilisateur(getPassword());
        user.setNumeroTelephoneUtilisateur(telephoneField.getText());
        user.setRoleUtilisateur(roleComboBox.getValue());
        user.setQuestionSecurite(securityQuestionComboBox.getValue());
        user.setReponseSecurite(securityAnswerField.getText());

        // Call the service to update the user.
        boolean success = utilisateurService.updateUtilisateur(user);
        if(success) {
            showAlert(Alert.AlertType.INFORMATION, "Succès", "L'utilisateur a été modifié avec succès.");
            // Close the update window.
            Stage stage = (Stage) nomField.getScene().getWindow();
            stage.close();
        } else {
            showGlobalError("Échec de la modification de l'utilisateur. Veuillez réessayer.");
        }
    }
