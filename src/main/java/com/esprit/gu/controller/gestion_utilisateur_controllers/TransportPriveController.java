...
public class TransportPriveController implements Initializable {
    ...
    @FXML
    private void navigateToScooter() {
        try {
            // Load the scooter view
            Parent root = FXMLLoader.load(getClass().getResource("/views/gestion_utilisateur_views/scooter.fxml"));
    ...
    @FXML
    private void navigateToTaxi() {
        try {
            // Load the taxi view
            Parent root = FXMLLoader.load(getClass().getResource("/views/gestion_utilisateur_views/taxi.fxml"));
            Stage stage = (Stage) welcomeLabel.getScene().getWindow();
            stage.setScene(new Scene(root));
        } catch (IOException e) {
            e.printStackTrace();
